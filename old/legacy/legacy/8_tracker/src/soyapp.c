...
sflag SOY_render(soyHandle hApp, int32 width, int32 height) {
  soyAppHandle app = (soyAppHandle)hApp;
  soyFont prevFont = app->draw.font;
  SOY_drawSetPaletteColour(app, SOY_GUI_NEUTRAL);
  SOY_moduleRender(app, &evolution);
  SOY_fontSelect(app, SOY_FONT_DEFAULT);
  if(!SOY_windowIsFullscreen(app)) {
    SOY_drawBaseGUI(app, width, height);
  }
  app->draw.font = prevFont;
  return TRUE;
}

sflag SOY_resize(soyHandle hApp, int32 width, int32 height) {
  soyAppHandle app = (soyAppHandle)hApp;
  soyFont prevFont = app->draw.font;
  SOY_drawSetPaletteColour(app, SOY_GUI_NEUTRAL);
  SOY_moduleResize(app, &evolution);
  SOY_moduleRender(app, &evolution);
  SOY_fontSelect(app, SOY_FONT_DEFAULT);
  if(app->hardwareAccelaration) {
    SGL_viewport(0, 0,
        SOY_width(app)*app->pixelResolution,
        SOY_height(app)*app->pixelResolution);
  }
  if(!SOY_windowIsFullscreen(app)) {
    SOY_drawBaseGUI(app, width, height);
  }
...
soyGLShader defShader;
soyGLTexture softTexture;
SOY_VAO triVAO;
SOY_VBO triVBO;
...
int32 main(int argc, char* argv[]) {
  uint32 appFrames = 0;
  SLOG2("ArgC: %d ArgV: %s\n", argc, argv[0]);
  SOY_appCreate(&app, 640, 480, 1, "soyTracker 1.12", FALSE);
  SOY_loadGUIPalette(&app, "assets/palettes/win2k.soy");
  SOY_windowSetResizeFunc(&app, SOY_resize);
  SOY_appSetTimestep(&app, 60);
  SOY_appSetRenderStep(&app, 1);
  SOY_appSetUpdateStep(&app, 1);
  SOY_initGUI(&app, app.frameBuffer.width, app.frameBuffer.height);
  SOY_printMemoryState(&app);
  SOY_enableSpriteGUI(&app, TRUE);
  SOY_moduleLoad(&tracker, "soytracker.dll", "tracker", &app);
  soy_module_construct(&app);
  /*
  SOY_moduleLoad(&evolution, "evolution.dll", "evolution", &app);
  */
  /*
  ...
  */
  SLOG("Loaded module\n");
  while(!SOY_appShouldQuit(&app)) {
    SOY_appPollInput(&app);
    SOY_appUpdateTiming(&app);
    SOY_appBeginFrame(&app);
    SOY_moduleReload(&app, &tracker);
    /*
    SOY_moduleReload(&app, &evolution);
    */
    SOY_moduleUpdate(&app, &tracker);
    SOY_moduleRender(&app, &tracker);
    soy_module_update(&app, &tracker);
    soy_module_render(&app, &tracker);
    /*
    SOY_moduleUpdate(&app, &evolution);
    */
    SOY_render((soyHandle)&app, app.frameBuffer.width, app.frameBuffer.height);
  ...
  SOY_moduleFree(&app, &tracker);  
  SOY_moduleFree(&app, &evolution);
  SOY_appDestroy(&app);
  return 0;
}
