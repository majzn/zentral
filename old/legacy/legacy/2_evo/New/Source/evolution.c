...
#define ENERGY_SPENT_DETECT 0.00001f
#define ENERGY_SPENT_INPUT 0.00001f
#define ENERGY_SPENT_COMPUTE 0.0001f
#define ENERGY_SPENT_OUTPUT 0.00001f
#define ENERGY_SPENT_MOVE 0.00001f
...
typedef struct tag_evoWorld {
  ...
  uint32 parentIndices[MAX_CELLS];
...
evoLink evo_link(evoGene gene) {
  ...
  s.weight = (((real32)((gene.data & GENE_WEIGHT) >> 8)-127)/255.0f)*4.0f;
...
evoSensor evo_sensor(cString label, uint32 gene) {
  evoSensor s;
  real32 angle = ((real32)(gene & GENE_ANGLE) / (real32)0xFF)*PAL_PI*2.0f;
  s.inputVal = 0.0f;
  s.outputVal = 0.0f;
  s.currentVal = 0.0f;
  s.label = label;
...
evoGenome evo_genome_inherit(evoGenome* a, evoGenome* b, uint32 numGenes, real32 mutation) {
  uint32 i;
  evoGenome g;
...
evoParams evo_params(evoGenome* genome, uint32 maxGenes) {
  evoParams p;
  uint32 i;
  p.activeLinkCount = 0;
  p.size = 1.5f;
  p.age = 0.0f;
  p.energy = 0.8f+(rand()%200)/2000.0f;
  p.fertility = 0.0f;
  p.clockRate = 0.01f;
  p.sensitivity = 0.5f;
  p.memory = 0.0f;
...
sflag evo_world_resize(palAppHandle app, evoWorld* w) {
  ...
  w->worldLeft = 32;
  w->worldRight = PAL_width(app)-400;
  w->worldUp = 64;
  ...
  w->tileWidth = (real32)w->worldWidth/WORLD_TILE;
  w->tileHeight = (real32)w->worldHeight/WORLD_TILE;
	return TRUE;
}
...
sflag evo_world_populate(evoWorld* w, sflag reset) {
  ...
  real32 randDir;
  uint32 i;
  uint32 worldWidth = (w->worldRight - w->worldLeft);
  uint32 worldHeight = (w->worldDown - w->worldUp);
  w->aliveCells = 0;
  if(!reset) {
    w->curSeed = w->curSeed;
  } else {
  ...
  if(reset) {
    for(i = 0; i < pal_min(w->maxCells, w->startCells); i++) {
      ...
      w->cells[i] = evo_cell(randX, randY,
        randDir, NULL, NULL, rand(), w->maxGenes, w->mutation, global_idCounter++, i);
    ...
    for(i = 0; i < pal_min(w->maxCells, w->startCells); i++) {
      ...
      w->cells[i] = evo_cell(randX, randY,
        ...
        &w->bestCells[rand()%(pal_min(i+1, FIT_SAMPLE_SIZE))],
        rand(), w->maxGenes, w->mutation, global_idCounter++, i);
      w->aliveCells++;
    }
  }
  w->generations++;
...
sflag evo_world_init(palAppHandle app,
    evoWorld* w, real32 mutation, uint32 maxGenes, uint32 maxCells, uint32 startCells, sflag reset) {
  uint32 i;
  w->friction = 0.001f;
  w->worldLeft = 32;
  w->worldRight = PAL_width(app)-400;
...
#define EVO_COL(a, b) evo_cell_check_collision(a, b)
...
real32 evo_sensor_update(evoWorld* world, evoCell* cell, evoSensor* sensor, evoTile* curTile) {
  ...
  switch(sensor->type) {
    ...
    default: { sensor->currentVal = 0.0f; break; }
  }
  sensor->outputVal = sensor->currentVal;
  return pal_min(1.0f, pal_max(0.0f, energySpent*cell->params.size*cell->params.size));
}
...
sflag evo_link_update(evoCell* cell, evoLink* link) {
  cell->params.sensors[link->outputID%MAX_SENSORS].inputVal +=
    cell->params.sensors[link->inputID%MAX_SENSORS].outputVal*link->weight*2;
  link->internalValue = cell->params.sensors[link->inputID%MAX_SENSORS].outputVal*link->weight*2;
  return TRUE;
}
...
evoCell* evo_cell_update(palAppHandle app, evoWorld* world, evoCell* cell, uint32 index) {
  ...
  uint32 colIndexY;
	real32 energySpent = 0.0f;
	real32 eatAmount = (cell->params.size*cell->params.size)*0.1f;
  uint32 tileIndexX = (uint32)((cell->physics.posX-world->worldLeft)/WORLD_TILE);
  uint32 tileIndexY = (uint32)((cell->physics.posY-world->worldUp)/WORLD_TILE);
  evoCell* curCell = NULL;
	evoCell* childParent = NULL;
  evoTile* curTile = NULL;
  cell->colliding = FALSE;
  cell->index = index;
  tileIndexX = pal_min(tileIndexX, world->tileWidth);
  tileIndexY = pal_min(tileIndexY, world->tileHeight);
  curTile = &world->worldTiles[tileIndexX+tileIndexY*((int32)world->tileWidth)];
  ...
  for(i = 0; i < world->aliveCells; i++) {
    if(world->cells[i].ID != cell->ID) {
      curCell = &world->cells[i];
			if(EVO_COL(cell, curCell)) {
        cell->collideIndex = i;
        cell->colliding = TRUE;
        if(cell->params.fertility > 0.5f &&
          ...
          cell->params.fertility *= 0.25f;
          cell->params.energy *= 0.75f;
          cell->params.size *= 0.75f;
          cell->children++;
          cell->score += world->rewardSex*100.0f*(real32)cell->children;
          world->cells[i].params.fertility *= 0.25f;
          world->cells[i].params.size *= 0.75f;
          world->cells[i].params.energy *= 0.75f;
          world->cells[i].children++;
          world->cells[i].score +=
						world->rewardSex*100.0f*(real32)world->cells[i].children;
        }
        break;
      }
    }
  ...
  for(i = 0; i < MAX_SENSORS; i++) {
    energySpent += evo_sensor_update(world, cell, &cell->params.sensors[i], curTile);
  }

  energySpent*=(0.001f+cell->params.age*(world->ageEffect)*0.1f);

  cell->score += world->rewardEat*eatAmount*curTile->foodValue;
  cell->score += world->rewardAge;
  cell->score += world->rewardProc*(energySpent/MAX_SENSORS);
  cell->score += world->rewardSize*cell->params.size;

  cell->params.temperature*=0.9f;
  cell->params.humidity*=0.9f;
  cell->params.energy += eatAmount*curTile->foodValue*(world->foodNutrition);
  cell->params.fertility += eatAmount*0.1f;
	cell->params.size += eatAmount*0.001f;
  curTile->foodValue = 0.0f;
  cell->params.temperature = curTile->seasonTemp;
  cell->params.humidity = curTile->seasonHumid;
  ...
  cell->params.size -= (energySpent/MAX_SENSORS);
  cell->params.size = pal_min(pal_max(0.1f, cell->params.size), world->sizeLimit);
  cell->params.energy -= (energySpent/MAX_SENSORS);
  cell->params.energy *= PAL_bellCurve(cell->params.temperature, -0.5f, 5.0f, 12);
...
sflag evo_world_tiles_update(evoWorld* w) {
  ...
  uint32 y;
  evoTile* curTile;
  uint32 tileWidth = w->worldWidth/WORLD_TILE;
  uint32 tileHeight = w->worldHeight/WORLD_TILE;
  real32 avgTemp = 0.0f;
  real32 avgHumid = 0.0f;
  real32 sinTemp = 1.0f-
    (w->seasonMod+(sin((((real32)w->age/24)/360.0f-0.23f)*PAL_PI*2))*(w->seasonMod*0.5f));
  real32 sinHumid = 1.0f-
    (w->seasonMod+(sin((real32)(w->age*0.000124))*
           cos((real32)(w->age*0.000001f))*
           sin((((real32)w->age/360)/666.0f)*PAL_PI*2))*1.0f)*(w->seasonMod*0.5f);
...
uint32 global_mousePrevX = 0;
uint32 global_mousePrevY = 0;
...
sflag evo_world_update(palAppHandle app, evoWorld* w) {
  ...
  real32 randDir = 0.0f;
  uint32 deadAmount = 0;
  uint32 mTX = (PAL_mouseX(app)-w->worldLeft)/WORLD_TILE;
  uint32 mTY = (PAL_mouseY(app)-w->worldUp)/WORLD_TILE;
  uint32 aliveCells = w->aliveCells;
  ...
  if((PAL_mouseLeft(app) || PAL_mouseRight(app)) &&
      mTX+(uint32)(dist*deltaX) < (w->worldWidth/WORLD_TILE) &&
      mTY+(uint32)(dist*deltaY) < (w->worldHeight/WORLD_TILE) &&
      mTX+(uint32)(dist*deltaX) > 0 &&
      mTY+(uint32)(dist*deltaY) > 0 &&
      PAL_keyPressed(app, PAL_SHIFT)) {
    while(dist > 0.0f) {
      if(PAL_mouseLeft(app)) {
        w->worldTiles[(mTX+(uint32)rX)+(mTY+(uint32)rY)*(w->worldWidth/WORLD_TILE)].wall = TRUE;
      } else {
        w->worldTiles[(mTX+(uint32)rX)+(mTY+(uint32)rY)*(w->worldWidth/WORLD_TILE)].wall = FALSE;
      }
      rX += deltaX;
      rY += deltaY;
      dist-= 1.0f;
  ...
  if(deadAmount >= aliveCells) {
		aliveCells = 0;
	} else {
		aliveCells -= deadAmount;
	}
  w->aliveCells = pal_min(aliveCells, MAX_CELLS);
  for(i = 0; i <  pal_min(w->maxCells, w->aliveCells); i++) {
    ...
    if(parentCell && numNew*2 < MAX_CELLS-2) {
  ...
  if(w->aliveCells) {
    for(j = 0; j < FIT_SAMPLE_SIZE; j++) {
      for(i = 0; i < FIT_SAMPLE_SIZE-j-1; i++) {
...
sflag global_sliderWidth = 200;

sflag evo_slider_draw(palAppHandle app, uint32 x, uint32 y, uint32 length, evoSlider* slider) {
  int8 strBufA[32];
  int8 strBufB[32];
  ...
  if(!slider->integer) {
    sprintf(strBufB, "%.02f", slider->outVal);
  } else {
    sprintf(strBufB, "%d", (int32)slider->outVal);
  }
  ...
  PAL_drawRectF(app, x+(uint32)slider->pixelOffset, y, x+(uint32)slider->pixelOffset+8, y+8);
  return TRUE;
}

sflag evo_world_draw(palAppHandle app, evoWorld* w) {
  uint32 i, j, x, y;
  uint32 inputSensorID;
  uint32 outputSensorID;
  uint8 foodCol;
  uint8 tempCol;
  uint8 rainCol;
  uint8 pherCol;
  ...
  uint32 tileWidth = (w->worldWidth)/WORLD_TILE;
  uint32 tileHeight = (w->worldHeight)/WORLD_TILE;
  uint32 circleWidth = 128;
  uint32 circlePosX = w->worldRight+circleWidth+24;
  uint32 circlePosY = w->worldDown-circleWidth-24;
  int32 inputPosX;
  int32 inputPosY;
  int32 outputPosX;
  int32 outputPosY;
...
sflag pal_module_construct(palApp* app) {
  ...
  global_evoWorld.worldDetail = 0.0f;
  global_evoWorld.featureSize = 0.0f;
  ...
  slider[sIndex++] = evo_slider("Size Limit", 0.0f, 32.0f, 0.5f);
  slider[sIndex++] = evo_slider("Age Effect", 0.0f, 1.0f, 0.05f);

  slider[sIndex++] = evo_slider("Food Value", 0.0f, 1.0f, 1.0f);
  slider[sIndex++] = evo_slider("Food Grow ", 0.0f, 1.0f, 0.2f);
  slider[sIndex++] = evo_slider("Food Decay", 0.0f, 1.0f, 0.1f);
  slider[sIndex++] = evo_slider("Food Sens.", 0.0f, 1.0f, 0.5f);

  slider[sIndex++] = evo_slider("Fr. Scale ", 0.0f, 1.0f, 0.15f);
  slider[sIndex++] = evo_slider("Fr. Iter. ", 0.0f, 10.0f, 0.75f);

  slider[sIndex++] = evo_slider("Avg. Temp.", 0.0f, 1.0f, 0.15f);
  slider[sIndex++] = evo_slider("Avg. Rain ", 0.0f, 1.0f, 0.5f);
  slider[sIndex++] = evo_slider("Season Mod", 0.0f, 1.0f, 0.1f);

  slider[sIndex++] = evo_slider("Score Eat ", 0.0f, 1.0f, 0.501f);
  slider[sIndex++] = evo_slider("Score Age ", 0.0f, 1.0f, 0.501f);
  slider[sIndex++] = evo_slider("Score Sex ", 0.0f, 1.0f, 1.0f);
  slider[sIndex++] = evo_slider("Score Move", 0.0f, 1.0f, 0.501f);
  slider[sIndex++] = evo_slider("Score Proc", 0.0f, 1.0f, 0.501f);
  slider[sIndex++] = evo_slider("Score Size", 0.0f, 1.0f, 0.501f);
  slider[sIndex++] = evo_slider("Score Kill", 0.0f, 1.0f, 0.5f);
...
sflag pal_module_update(palApp* app) {
  ...
  w->simSpeed = (uint32)slider[sIndex++].outVal;
  w->startCells = (uint32)slider[sIndex++].outVal;
  w->maxCells = (uint32)slider[sIndex++].outVal;
  w->maxGenes = (uint32)slider[sIndex++].outVal;
  ...
  w->foodDecay = (slider[sIndex++].outVal*40.0f);
  w->foodRange = (slider[sIndex++].outVal)*25.0f;

	if(w->featureSize != slider[sIndex].outVal) {
		evo_world_tiles_init(w, 0, w->curSeed);
	}
  w->featureSize = slider[sIndex++].outVal;

	if(w->worldDetail != slider[sIndex].outVal) {
		evo_world_tiles_init(w, 0, w->curSeed);
	}
	w->worldDetail = slider[sIndex++].outVal;

  w->tempMod = slider[sIndex++].outVal*5.0f;
  w->humidMod = slider[sIndex++].outVal*5.0f;
  w->seasonMod = slider[sIndex++].outVal;

  w->rewardEat = slider[sIndex++].outVal - 0.5f;
  w->rewardAge = slider[sIndex++].outVal - 0.5f;
  w->rewardSex = slider[sIndex++].outVal - 0.5f;
  w->rewardMove = (slider[sIndex++].outVal - 0.5f)*100.0f;
  w->rewardProc = slider[sIndex++].outVal - 0.5f;
  w->rewardSize = slider[sIndex++].outVal - 0.5f;
  w->rewardKill = slider[sIndex++].outVal - 0.5f;
  ...
  while(updateSteps > 0 && w->simPlaying) {
    ...
    if(w->aliveCells < 2 && w->maxCells > 0) {
